...
class RoomView(APIView):
    ...
    def get(request):
        ...
        try:
            ...
            for room in rooms:
                room['member_count'] = len(room['members']) + 1
                room.pop('members')
    ...
    def post(request):
        ...
        try:
            ...
            if member_list is not None:
                q = Q()
                [q.add(Q(pk=m), Q.OR) for m in member_list if m != request.user.pk]
                if len(q) > 0:
                    member_list = Member.objects.filter(q).distinct()
...
class SingleRoomView(APIView):
    ...
    def put(request, pk):
        ...
        try:
            ...
            if request.user != room_config.master:
...
class RoomMemberView(APIView):
    ...
    def post(request, pk):
        ...
        try:
            ...
            member_list = request.data.get('member_list')
            member_list = [get_object_or_404(Member, pk=m) for m in member_list if m != room_config.master.pk]
    ...
    def delete(request, pk):
        ...
        try:
            ...
            member_list = request.data.get('member_list')
            [room.members.remove(get_object_or_404(Member, pk=m)) for m in member_list]
